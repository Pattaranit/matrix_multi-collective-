...
int main(int argc, char *argv[]) {
	...
	if(rank!=0)
	{
		...
		/* write file */
		fpO = fopen("D:\\3rdYrs\\first term\\CPE374 Parallel\\Lab\\MatrixMultipleDataSet_Small\\result_S.txt", "w");
		//fpO = fopen("matR.txt", "w");

		for (i=0; i<nrows1; i++) {
			for (j=0; j<ncolumns2; j++) {
				fprintf(fpO,"%d ",matR[i][j]);
				//printf("%d ",matR[i][j]);
			}
			fprintf(fpO,"\n");
		}
